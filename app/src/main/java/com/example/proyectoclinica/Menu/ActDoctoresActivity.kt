...
class ActDoctoresActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val vespecialidad_id = bundle.getString("especialidad_id").toString()
        val vhorario_id = bundle.getString("horario_id").toString()
        val vfoto = bundle.getString("foto").toString()
        setContent {
            ...
            var especialidad_id by remember { mutableStateOf(vespecialidad_id) }
            var horario_id by remember { mutableStateOf(vhorario_id) }
            var foto by remember { mutableStateOf(vfoto) }
            ...
            Column (
                ...
                TextField(value=foto,
                    ...
                    shape = RoundedCornerShape(24.dp),
                    label ={Text(text="Foto del doctor")},
                    onValueChange = {
                        foto=it
                    })
                Spacer(modifier= Modifier.padding(all= dimensionResource(id = R.dimen.espacio_min)))
                Button(onClick = {
                    leerServicioActualizar(dni,nombre_completo,correo,telefono,especialidad_id,horario_id,foto)
                }) {
                    Text(text = "Actualizar")
                }
                Button(onClick = {
                    leerServicioEliminar(dni,nombre_completo,correo,telefono,especialidad_id,horario_id,foto)
                }) {
                    Text(text = "Eliminar")
                }
            }
    ...
    private fun leerServicioActualizar(dni: String, nombreCompleto: String, correo: String, telefono: String,especialidad_id:String,horario_id:String, foto:String) {
        // Instantiate the RequestQueue.
        val queue = Volley.newRequestQueue(this)
        val url = Total.rutaServices+"doctoresupdate.php"
        // Request a string response from the provided URL.
        val stringRequest = object : StringRequest(
            Request.Method.POST, url,
            { response ->
                Log.d("DATOS",response)
                Toast.makeText(this,"Se ha actualizado doctor con DNI:"+response,
    ...
    private fun leerServicioEliminar(dni: String, nombreCompleto: String, correo: String, telefono: String,especialidad_id:String,horario_id:String, foto:String) {
        // Instantiate the RequestQueue.
        val queue = Volley.newRequestQueue(this)
        val url = Total.rutaServices+"doctoresdelete.php"
        // Request a string response from the provided URL.
        val stringRequest = object : StringRequest(
            Request.Method.POST, url,
            { response ->
                Log.d("DATOS",response)
                Toast.makeText(this,"Se ha eliminado doctor con DNI:"+response,
                    Toast.LENGTH_SHORT).show()
                startActivity(Intent(this,BuscarDoctorActivity::class.java))
            },
            {
